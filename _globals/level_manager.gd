...
func handle_explosion(explosion_data: Dictionary) -> void:
	...
	var radius: float = float(explosion_data.get("radius", 0.0))
	var damage: int = int(explosion_data.get("damage", 0))
	var tree := get_tree()
	if tree and damage > 0 and radius >= 0.0:
		for node in tree.get_nodes_in_group("players"):
			if not (node is Player) or not is_instance_valid(node):
				continue
			var paper: Player = node as Player
			if paper.global_position.distance_to(origin) > radius:
				continue
			paper.apply_explosion_damage(damage)
			show_damage_number.rpc(damage, paper.global_position, paper.sync_color)
		for node in tree.get_nodes_in_group("buildings"):
			if not (node is Building) or not is_instance_valid(node):
				continue
			var building: Building = node as Building
			if not _explosion_hits_building(building, origin, radius):
				continue
			building.apply_explosion_damage(damage)
	DmManager.apply_everything_burns(explosion_data)


func _explosion_hits_building(building: Building, origin: Vector2, radius: float) -> bool:
	if building.global_position.distance_to(origin) <= radius:
		return true
	var hull: Rect2 = building.raid_hull_rect(0.0)
	var closest := Vector2(
		clampf(origin.x, hull.position.x, hull.end.x),
		clampf(origin.y, hull.position.y, hull.end.y)
	)
	return closest.distance_to(origin) <= radius
