...
- Tile-art owner + MultiplayerSynchronizer / snapshot RPC (match US-001 T008 / US-024 bounds)
- Do not send dungeon restyles (there are none)

## Requirements

- FR-005, MR-001, MR-002
- Snapshot: kind + variety index + presentation (Neutral / Reality / Fantasy) for each outside tile.
- Dungeon tiles are not restyled by this story and need not be in the snapshot.

## Acceptance

- **Given** a host conversion, **When** peers are in session, **Then** they show the same Reality-element variant.
