...
Replace the stub `gui/dm/skill_tree.tscn` with **DM** + **Dad** tabs, **3×3 + ultimate** each, hover tooltips. **No** spend, unlock power, or spawns. James signed at `e264851`. Defaults in the story **Open defaults** table are locked unless overridden. Entry point today: Skill Tree doodad → `DmManager._show_skill_tree_hud` → `DmHud._toggle_skill_tree_hud`.
