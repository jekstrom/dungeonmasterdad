...
const DM_PASSIVES: Array[Dictionary] = [
	...
	{"id": "blind_one_legged_monkeys", "name": "Blind one-legged monkeys", "effect": "Increase gremlin movement speed", "row": "Gremlins"},
	{"id": "crib_death", "name": "Crib Death", "effect": "Automatically summon 1 gremlin from the dungeon exit every min, but they only live for 1 minute.", "row": "Gremlins"},
...
func _style_tab_button(btn: TextureButton, active: bool) -> void:
	...
	if active:
		...
		# Slight overlap onto TabShelf so active blends into the page.
		btn.offset_bottom = 4
	else:
